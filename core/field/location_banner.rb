module PokeAccess
  ...
  module LocationBanner
    # Registers the LocationWindow reader for a game profile. The plugin's own off switch is honoured: its
    # initialize returns immediately when jess_letreros_activo is set, so events that suppress the banner
    # (pb_jla) build the object and draw nothing -- and the hook, which fires either way, announced a sign
